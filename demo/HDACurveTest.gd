...
func cook():
	...
	if id == -1 and stash.size() > 0:
		await _await_task(hego_asset_node.set_preset(stash))
	...
	await _await_task(hego_curve_input_node.instantiate())
	await _await_task(hego_curve_input_node.set_curve_from_path_3d(input_node, 2))
	await _await_task(hego_asset_node.connect_input(hego_curve_input_node, 0))
