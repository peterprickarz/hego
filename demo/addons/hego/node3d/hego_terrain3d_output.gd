...
static func handle(context: HEGoOutputContext) -> void:
	...
	var layers: Variant = await context.await_task(context.asset.get_heightfield_layers(PackedStringArray(REQUESTED_ATTRIBS)))
	...
	var terrain := _spawn_terrain(context, config)
	if terrain == null:
		return
	...
	if texture_setup["enabled"]:
		texture_setup = await _apply_texture_assets(context, terrain, texture_setup)

	var hole_image: Image = null
	if texture_setup["enabled"]:
		var hole_layer := HEGoTerrain3DUtil.get_layer_by_name(layers, HOLE_LAYER)
		hole_image = await _fetch_layer_image(context, hole_layer)
	...
	var height_image := await _fetch_layer_image(context, height_layer)
	if height_image == null:
		HEGoLog.get_singleton().error(LOG_CATEGORY, "Failed to fetch height image for Terrain3D output.")
		return

	var region_map_image := await _fetch_layer_image(context, HEGoTerrain3DUtil.get_layer_by_name(layers, REGION_MAP_LAYER))

	var color_images := {}
	for channel in COLOR_LAYERS.keys():
		color_images[channel] = await _fetch_layer_image(context, HEGoTerrain3DUtil.get_layer_by_name(layers, COLOR_LAYERS[channel]))
...
static func _spawn_terrain(context: HEGoOutputContext, config: Dictionary) -> Node:
	...
	# rebuilds that subtree, and a terrain must outlive it. context.host exists for this.
	var parent_node := context.ensure_parent(context.host, path_parts)
...
static func _apply_texture_assets(context: HEGoOutputContext, terrain: Node, texture_setup: Dictionary) -> Dictionary:
	var disabled := {"enabled": false, "layers": [], "lowest_slot": -1, "util": null}
	...
	for texture_layer in texture_setup["layers"]:
		...
		var weight_image := await _fetch_image_for_part(context, int(texture_layer["part_id"]))
...
static func _fetch_layer_image(context: HEGoOutputContext, layer: Dictionary) -> Image:
	if layer.is_empty() or not layer.has("part_id"):
		return null
	return await _fetch_image_for_part(context, int(layer["part_id"]))


static func _fetch_image_for_part(context: HEGoOutputContext, part_id: int) -> Image:
	if part_id < 0:
		return null
	var image: Variant = await context.await_task(context.asset.fetch_heightfield_layer_image(part_id))
	if image == null:
		return null
