...
func _ready():
	if not label or not slider or not spin_box:
		push_error("FloatParmUI: One or more nodes are null. Check scene structure: Label=%s, HSlider=%s, SpinBox=%s" % [label, slider, spin_box])
		return
...
func setup(_param: Dictionary):
	initializing = true
	if not label:
		push_error("FloatParmUI: Label node is null. Cannot set label text.")
		return
	if not slider or not spin_box:
		push_error("FloatParmUI: Slider or SpinBox node is null. Cannot set values.")
		return
	slider.step = 0.01
	spin_box.step = 0.01
	...
	slider.max_value = param.get("max", 10.0)
	spin_box.min_value = param.get("min", 0.0)
	spin_box.max_value = param.get("max", 10.0)
	...
	slider.value = param.get("values", [0.0])[0]
	spin_box.value = param.get("values", [0.0])[0]
...
func _on_slider_value_changed(value: float):
	if not spin_box:
		push_error("FloatParmUI: SpinBox node is null. Cannot update value.")
		return
	if initializing:
		return
...
func _on_spin_box_value_changed(value: float):
	if not slider:
		push_error("FloatParmUI: Slider node is null. Cannot update value.")
		return
	if initializing:
		return
	print(value)
