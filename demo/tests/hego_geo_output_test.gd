...
func _initialize() -> void:
	# --- classes registered ------------------------------------------------
	check(ClassDB.class_exists("HEGoPointOutput"), "HEGoPointOutput is registered")
	check(ClassDB.class_exists("HEGoPointSelection"), "HEGoPointSelection is registered")
	...
	for method in ["is_valid", "get_point_count", "get_attribute_names", "has_attribute",
		...
		check(has_method_bound("HEGoPointOutput", method), "HEGoPointOutput.%s is bound" % method)

	for method in ["size", "get_indices", "filter_by", "split_by", "get_points"]:
		check(has_method_bound("HEGoPointSelection", method), "HEGoPointSelection.%s is bound" % method)

	check(has_method_bound("HEGoAssetNode", "get_point_output"), "HEGoAssetNode.get_point_output is bound")

	# --- the surface side ---------------------------------------------------
	check(ClassDB.class_exists("HEGoSurfaceOutput"), "HEGoSurfaceOutput is registered")
	check(ClassDB.class_exists("HEGoSurfaceSelection"), "HEGoSurfaceSelection is registered")

	for method in ["is_valid", "get_primitive_count", "get_attribute_names", "has_attribute",
			"get_attribute_names_with_prefix", "load_attributes", "get_attribute",
			"select_all", "filter_by", "split_by"]:
		check(has_method_bound("HEGoSurfaceOutput", method), "HEGoSurfaceOutput.%s is bound" % method)

	for method in ["size", "get_indices", "filter_by", "split_by", "get_surface"]:
		check(has_method_bound("HEGoSurfaceSelection", method), "HEGoSurfaceSelection.%s is bound" % method)

	check(has_method_bound("HEGoAssetNode", "get_surface_output"), "HEGoAssetNode.get_surface_output is bound")

	var surfaces := HEGoSurfaceOutput.new()
	...
	check(HEGoSurfaceSelection.new().get_surface().is_empty(), "a bare prim selection yields no surface")

	# --- owner constants ----------------------------------------------------
	# HAPI_AttributeOwner: VERTEX 0, POINT 1, PRIM 2, DETAIL 3.
	check(HEGoPointOutput.OWNER_VERTEX == 0, "OWNER_VERTEX matches HAPI_ATTROWNER_VERTEX")
	check(HEGoPointOutput.OWNER_POINT == 1, "OWNER_POINT matches HAPI_ATTROWNER_POINT")
	check(HEGoPointOutput.OWNER_PRIM == 2, "OWNER_PRIM matches HAPI_ATTROWNER_PRIM")
	check(HEGoPointOutput.OWNER_DETAIL == 3, "OWNER_DETAIL matches HAPI_ATTROWNER_DETAIL")

	# --- empty states -------------------------------------------------------
	var output := HEGoPointOutput.new()
	...
	var selection := HEGoPointSelection.new()
	...
	var task: HEGoTask = node.get_point_output()
	check(task != null, "get_point_output returns a task")
	check(task.get_status() == HEGoTask.FAILED, "get_point_output fails without an instantiated HDA")
