...
namespace Geo
{
godot::Dictionary HEGo::Util::Geo::fetch_points(
		HEGoSessionManager *session_mgr, HAPI_NodeId node_id, godot::Ref<godot::Resource> fetch_point_config)
{
	...
	godot::Array positions = HEGo::Util::Attribs::fetch_vector3(
			session_mgr->get_session(), mesh_geo_info, mesh_part_info, HAPI_ATTROWNER_POINT, "P");
	...
	for (int i = 0; i < read_attribs.size(); i++)
	{
		...
		godot::Array values = HEGo::Util::Attribs::fetch_by_name(session_mgr->get_session(), mesh_geo_info,
				mesh_part_info, HAPI_ATTROWNER_POINT, attr_name.utf8().get_data());
	...
	if (mesh_part_info.vertexCount > 0)
	{
		HOUDINI_CHECK_ERROR(HoudiniApi::GetVertexList(
				session_mgr->get_session(), node_id, 0, vertex_point_indices.data(), 0, mesh_part_info.vertexCount));
	}
	...
	godot::Dictionary split_point_dictionary = HEGo::Util::Geo::build_nested_dictionary(
			split_attribs, split_attribs_dict, filtered_indices, read_attribs_dict, 0);
